        ...
        match message {
            ...
            ValidateTemplate(block_template) => {
                ...
                let status = block_template.header.prev_block_hash
                    == blockchain.blocks().last().map(|last_block| last_block.hash()).unwrap_or(Hash::zero());
            ...
            FetchTemplate(pubkey) => {
                ...
                transactions.extend(blockchain
                    .mempool()
                    .iter()
                ...
                // todo refactor so we don't have to rerun calculate
