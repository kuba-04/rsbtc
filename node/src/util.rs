...
// TODO possible improvement: someone can attach with very long chain node!!
pub async fn find_longest_chain_node() -> Result<(String, u32)> {
    println!("finding nodes with the largest blockchain length");
    ...
    for node in all_nodes {
        ...
        match message {
            Message::Difference(count) => {
                println!("receive Difference from {}", node);
...
pub async fn populate_connections(nodes: &[String]) -> Result<()> {
    ...
    for node in nodes {
        ...
        println!("send DiscoverNodes to {}", node);
        let message = Message::receive_async(&mut stream).await?;
        match message {
...
// TODO possible improvement: add another type to fetch entire chain instead of individual blocks
