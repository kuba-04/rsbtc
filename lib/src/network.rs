...
pub enum Message {
    /// fetch all utxos belonging to a pub key
    FetchUTXOs(PublicKey),
    /// utxos belonging to a pub key, Bool determines if marked
    UTXOs(Vec<(TransactionOutput, bool)>),
    /// send a transaction to the netwoerk
    SubmitTransaction(Transaction),
    /// broadcast a new transaction to other nodes
    NewTransaction(Transaction),
    /// Ask the node to prepare the optimal block template with the coinbase tx
    /// paying the specified pub key
    FetchTemplate(PublicKey),
    Template(Block),
    /// Ask the node to validate a block template.
    /// This is to prevent the node from mining an invalid block (if one has been found in the
    /// meantime or if transactions have been removed from the mempool)
    ValidateTemplate(Block),
    TemplateValidity(bool),
    SubmitTemplate(Block),
    /// Ask a node to report al teh other nodes it knows about
    DiscoverNodes,
    ...
    FetchBlock(usize),
    /// Broadcast a new bock to other nodes
    NewBlock(Block),
}
impl Message {
